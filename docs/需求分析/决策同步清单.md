...
| D7 | 模型供应商与配置 | 抽象到专门的 AI gateway。业务服务通过 OpenAI-compatible API 调用，首期各组无需各自实现供应商适配层；通过 config 修改模型 `baseURL` 和 `apiKey`。 | 知识、QA、报告配置 API；模型调用边界；后续 `ai-gateway` 服务设计 |
...
- AI gateway 首期只要求 OpenAI-compatible `baseUrl`、`apiKey`、模型名、超时配置；健康检查可通过后续运维接口增强。
