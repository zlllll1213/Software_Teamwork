...
| PR #224 `docs(services): add implementation status docs` | open | 当前文档 PR，负责把实现状态、缺口和新增文档补齐。 |
...
| PR #217 `feat(qa): implement B-03 QA Agent Run MVP with termination handling` | open | QA 完整 Agent Run MVP 仍按待合入能力处理。 |
...
| QA 会话、消息、配置、引用和统计资源 | 部分实现 | `services/qa/` 会话/消息 API、SSE 事件、config versions、citations、retrieval test/metrics 资源、AI Gateway chat client。 | Gateway QA active paths；QA service OpenAPI。 | 完整 Agent Run MVP、termination、真实 Knowledge retrieval、权限一致性和跨服务 smoke 待收口。 | #157、#217、#219、#210 |
...
| Document report files、settings、statistics、operation logs | 占位 / 待合入 | 当前 implementation 标记 report files/statistics/settings 仍为 scaffold；#221/#223 open。 | Gateway active document paths 已声明。 | DOCX 导出、文件内容读取、配置持久化、统计和日志未进入当前 `develop`。 | #159、#160、#221、#223 |
...
4. 测试策略要把 Go、migration、Gateway contract、frontend check/build 和 env-gated integration tests 放在一张可执行清单里。
