...
| LLM 配置 | 管理模型供应商、API 地址、模型名称、脱敏密钥引用、超时和生成参数。 |
...
qa service
   ...
   +--> LLM provider through configured model endpoint
```
...
| `GET/PUT /api/admin/llm-config` | `GET /api/v1/llm-config-versions/current`, `POST /api/v1/llm-config-versions` | `qa` | LLM 配置采用版本化资源，密钥只返回脱敏信息。 |
| `POST /api/admin/llm-config/test` | `POST /api/v1/llm-connection-tests` | `qa` | 创建一次 LLM 连接测试记录。 |
...
QA 不得向前端返回完整 API key。数据库中只保存 `api_key_secret_ref` 和 `api_key_last4`，密钥正文应由密钥服务或部署环境管理。
...
创建一次 LLM 连接测试记录。该接口不应保存明文 API key 到日志或响应。
...
内部服务错误也必须被 gateway 归一化为公开错误 envelope，不能把 SQL 错误、LLM provider 原始错误、密钥引用、prompt 或内部 URL 直接返回给前端。
