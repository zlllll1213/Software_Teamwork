...
import type { KnowledgeConfigResponse } from '@/api/admin'
import { getKnowledgeConfig } from '@/api/admin'
...
export function KnowledgeConfig() {
  const { data, isLoading, isError, error } = useQuery<KnowledgeConfigResponse>({
    queryKey: ['admin', 'knowledge-config'],
    ...
    <div>
      ...
      {isError && (
        <div className="mb-6 rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-sm text-destructive">
          加载知识配置失败: {error instanceof Error ? error.message : '未知错误'}
        </div>
      )}
                ...
                {data.knowledge_bases.map((kb) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">文档数: {kb.doc_count}</p>
                    </div>
                    <Badge variant={kb.status === 'active' ? 'default' : 'secondary'}>
                      {kb.status === 'active' ? '活跃' : '停用'}
                    </Badge>
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-lg border border-border bg-card p-6">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-3">
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className="text-sm font-medium text-foreground">{data.defaults.top_k}</span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-sm text-muted-foreground">similarity_threshold</span>
                <span className="text-sm font-medium text-foreground">
                  {data.defaults.similarity_threshold}
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-sm text-muted-foreground">use_rerank</span>
                <Badge variant={data.defaults.use_rerank ? 'default' : 'secondary'}>
                  {data.defaults.use_rerank ? '是' : '否'}
                </Badge>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-sm text-muted-foreground">rerank_threshold</span>
                <span className="text-sm font-medium text-foreground">
                  {data.defaults.rerank_threshold}
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-sm text-muted-foreground">rerank_top_n</span>
                <span className="text-sm font-medium text-foreground">
                  {data.defaults.rerank_top_n}
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-sm text-muted-foreground">默认知识库</span>
                <span className="text-sm font-medium text-foreground">
                  {data.defaults.knowledge_bases.length > 0
                    ? data.defaults.knowledge_bases.join(', ')
                    : '无'}
                </span>
              </div>
            </div>
